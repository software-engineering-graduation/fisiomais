...
const NovoTratamento = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const resExercicios = await axios.get('http://localhost:8081/api/exercicio/available', {
  ...
  const columns = [
    ...
    { title: 'Descrição', dataIndex: 'descricao', key: 'descricao' },
    {
        ...
        key: 'midia',
        render: (_, record) => (
            <a href={record.midias[0]?.linkArquivo} target="_blank" rel="noopener noreferrer">
    ...
    },
    {
        ...
        key: 'acao',
        render: (_, record) => (
            <Button icon={<DeleteOutlined />} onClick={() => removerExercicio(record.id)}>
                Remover
            </Button>
        ),
    },
...
const onFinish = async (values) => {
  ...
  const adicionarExercicio = (exercicioId) => {
    ...
    if (exercicio) {
      setExerciciosSelecionados(prevExercicios => [...prevExercicios, exercicio]);
    }
  };

  const removerExercicio = (exercicioId) => {
    setExerciciosSelecionados(exerciciosSelecionados.filter(e => e.id !== exercicioId));
  };
